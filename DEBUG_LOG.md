...
- [x] 調査4: 根本原因特定 → flex全排除 → **CSSが反映されず失敗**
- [x] 調査5: **JSから直接インラインスタイルで強制注入**（CSS反映に依存しない）
  - `auth.js`: モーダル表示時に `body.style.height = (card - header)px` を直接設定
  - `admin.js`: タブ切り替え時にも同様の強制設定を実行
  - 全ファイルのバージョンパラメータを `?v=20260402` に更新
- [ ] ユーザー確認待ち
