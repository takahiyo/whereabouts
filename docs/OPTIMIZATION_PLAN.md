# スリム化・最適化の段階的計画（D1主軸 + Workersキャッシュ）
...
## 前提（構成）
- Plan A: Cloudflare Workers + D1 を主軸とする。
- Plan B: Workers 側のキャッシュ/リトライ制御を継続的に強化する。
...
2. **差分取得とキャッシュの最適化**
   - 画面非表示/タブ離脱時はポーリング間隔を延長。
   - 同一データへの重複取得を削除（通知/ツールなどのセクション単位で確認）。
   - 目的: 無駄な read / 更新の削減。
...
2. **設定・コンフィグの集約**
   - D1 側の設定、Cloudflare Workers 側の設定の重複を洗い出す。
   - どの値が「唯一の正」のソースかを明文化。
...
- [x] ファイルのアーカイブ/削除を実施 (archive/gas/ へ移動完了)
